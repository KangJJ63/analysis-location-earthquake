...
Author: NY.Kim
Date : 2019-12-23
특정 구 기준 행정동 생활인구 EDA결과 plot저장 Program
"""
import sys
import platform
...
try:
    os.makedirs("C:/earthquake/result/EDA/옥외대피장소/{0}".format(gu))
except OSError:
    pass
...
a = df_living["INDEX_ID"].unique()  # 미확인 격자 체크
...
df_living_f = df_living_f.loc[df_living_f["sigungu_nm"] == gu]
...
try:
    ...
    dong_p.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_주민_생활인구_비교{1}.csv".format(gu, today), encoding="cp949")
    ...
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_주민_생활인구_비교{1}.png".format(gu, today), bbox_inches='tight')
    plt.close()

    dong_p = living_index_max.groupby(["adm_dr_nm"], as_index=False)[["lbl", "refuge_sum"]].sum()
    dong_p.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_주민등록인구_옥외대피_비교{1}.csv".format(gu, today), encoding="cp949")
    ...
    for t, l in zip(leg.texts, new_labels): t.set_text(l)
    plt.title("행정동별 주민등록인구/옥외대피장소 비교", fontsize=15, y=1.03)
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_주민등록인구_옥외대피_비교{1}.png".format(gu, today))
    plt.close()

    dong_p = living_index_max.groupby(["adm_dr_nm"], as_index=False)[["val", "refuge_sum"]].sum()
    dong_p.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_생활인구_옥외대피_비교{1}.csv".format(gu, today), encoding="cp949")
    ...
    for t, l in zip(leg.texts, new_labels): t.set_text(l)
    plt.title("행정동별 생활인구/옥외대피장소 비교", fontsize=15, y=1.03)
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_생활인구_옥외대피_비교{1}.png".format(gu, today))
    plt.close()

    dong_p = living_index_max.groupby(["adm_dr_nm"], as_index=False)[["lbl", "po_sum"]].sum()
    dong_p.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_주민등록인구_수용가능_비교{1}.csv".format(gu, today), encoding="cp949")
    dong = dong_p.sort_values(by=["lbl"], ascending=False).adm_dr_nm.tolist()
    dong_p = pd.melt(dong_p, id_vars="adm_dr_nm", var_name="class", value_name="people")
    ...
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_주민등록인구_수용가능_비교{1}.png".format(gu, today), bbox_inches='tight')
    plt.close()

    dong_p = living_index_max.groupby(["adm_dr_nm"], as_index=False)[["val", "po_sum"]].sum()
    dong_p.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_생활인구_수용가능_비교{1}.csv".format(gu, today), encoding="cp949")
    dong = dong_p.sort_values(by=["val"],ascending=False).adm_dr_nm.tolist()
    dong_p = pd.melt(dong_p, id_vars="adm_dr_nm", var_name="class", value_name="people")
    plt.rcParams["figure.figsize"] = (12,8)
    g = sns.catplot(x="adm_dr_nm", y="people", hue="class", data=dong_p, kind="bar",
    ...
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동별_생활인구_수용가능_비교{1}.png".format(gu, today), bbox_inches='tight')
    plt.close()
    ...
    plt.xlabel("시간")
    plt.ylabel("")
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/총_생활인구_양상{1}.png".format(gu, today))
    plt.close()
    ...
    living_month_time.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_생활인구_현황{1}.csv".format(gu, today), encoding="cp949")
    ...
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_생활인구_현황{1}.png".format(gu, today))
    plt.close()

    living_month_time = df_living_f.groupby(["month", "TIME"], as_index=False)["H_MAX"].sum()
    living_month_time["month"] = living_month_time["month"].map(lambda x: str(x) + "월").astype("object")
    living_month_time.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_거주인구_현황{1}.csv".format(gu, today), encoding="cp949")
    sns.lineplot(x="TIME", y="H_MAX", hue="month", data=living_month_time, palette="Paired")
    plt.title("월/시간대 별 거주인구 현황", fontsize=15, y=1.03)
    plt.ylabel("")
    plt.xlim(0, 23)
    plt.legend().texts[0].set_text('')
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_거주인구_현황{1}.png".format(gu, today))
    plt.close()

    living_month_time = df_living_f.groupby(["month", "TIME"], as_index=False)["W_MAX"].sum()
    living_month_time["month"] = living_month_time["month"].map(lambda x: str(x) + "월").astype("object")
    living_month_time.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_근무인구_현황{1}.csv".format(gu, today), encoding="cp949")
    sns.lineplot(x="TIME", y="W_MAX", hue="month", data=living_month_time, palette="Paired")
    plt.title("월/시간대 별 근무인구 현황", fontsize=15, y=1.03)
    plt.ylabel("")
    plt.xlim(0, 23)
    plt.legend().texts[0].set_text('')
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_근무인구_현황{1}.png".format(gu, today))
    plt.close()

    living_month_time = df_living_f.groupby(["month", "TIME"], as_index=False)["V_MAX"].sum()
    living_month_time["month"] = living_month_time["month"].map(lambda x: str(x) + "월").astype("object")
    living_month_time.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_방문인구_현황{1}.csv".format(gu, today), encoding="cp949")
    sns.lineplot(x="TIME", y="V_MAX", hue="month", data=living_month_time, palette="Paired")
    plt.title("월/시간대 별 방문인구 현황", fontsize=15, y=1.03)
    plt.ylabel("")
    plt.xlim(0, 23)
    plt.legend().texts[0].set_text('')
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_방문인구_현황{1}.png".format(gu, today))
    plt.close()

    living_month_time = df_living_f.groupby(["month", "TIME"], as_index=False)["F_MAX"].sum()
    living_month_time["month"] = living_month_time["month"].map(lambda x: str(x) + "월").astype("object")
    living_month_time.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_외국인인구_현황{1}.csv".format(gu, today), encoding="cp949")
    sns.lineplot(x="TIME", y="F_MAX", hue="month", data=living_month_time, palette="Paired")
    plt.title("월/시간대 별 외국인 현황", fontsize=15, y=1.03)
    plt.ylabel("")
    plt.xlim(0, 23)
    plt.legend().texts[0].set_text('')
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/월_시간대_외국인인구_현황{1}.png".format(gu, today))
    plt.close()

    living_time_index = df_living_f.loc[df_living_f.groupby(["INDEX_ID", "TIME"])["A_MAX"].idxmax()]
    living_time_dong = living_time_index.groupby(["adm_dr_nm", "TIME"], as_index=False)["A_MAX"].sum()
    living_time_dong.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동_시간대별_생활인구_현황{1}.csv".format(gu, today), encoding="cp949")
    sns.lineplot(x="TIME", y="A_MAX", hue="adm_dr_nm", data=living_time_dong, palette="Paired")
    plt.title("행정동/시간대 별 생활인구 현황", fontsize=15, y=1.03)
    plt.xlim(0, 23)
    plt.ylabel("")
    plt.legend(bbox_to_anchor=(1.04, 1), loc="upper left").texts[0].set_text('행정동')
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동_시간대별_생활인구_현황{1}.png".format(gu, today), bbox_inches="tight")
    plt.close()

    living_month_index = df_living_f.loc[df_living_f.groupby(["INDEX_ID", "STD_YM"])["A_MAX"].idxmax()]
    living_month_dong = living_month_index.groupby(["adm_dr_nm", "STD_YM"], as_index=False)["A_MAX"].sum()
    living_month_dong["STD_YM"] = living_month_dong["STD_YM"].astype("str").map(lambda x: x[:-3])
    living_month_dong.to_csv("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동_월별_생활인구_현황{1}.csv".format(gu, today), encoding="cp949")
    sns.lineplot(x="STD_YM", y="A_MAX", hue="adm_dr_nm", data=living_month_dong, palette="Paired")
    plt.title("행정동/월 별 생활인구 현황", fontsize=15, y=1.03)
    plt.xlabel("")
    plt.xticks(rotation=25)
    plt.ylabel("")
    plt.legend(bbox_to_anchor=(1.04, 1), loc="upper left").texts[0].set_text('행정동')
    plt.savefig("C:/earthquake/result/EDA/옥외대피장소/{0}/행정동_월별_생활인구_현황{1}.png".format(gu, today), bbox_inches="tight")
    plt.close()
